...
for r in range(RoundNum):
    ...
    # vanilla algorithm
    if str(sys.argv[2]) == 'vanilla':
        new_neighbor=algvanilla.VanillaSelection(G, OutNeighbor, len_of_neigh,
                                                  len_of_test, str(sys.argv[6]),
                                                    LinkDelay, NodeDelay, IncomingLimit,
                                                      NodeHash,  NeighborSets,
                                                        r, sys.argv[4], IncomingNeighbor)
    # ucb algorithm
        ...
        [new_neighbor, hist_score_table, hist_score_length]= algucb.UCBelection(
            G, OutNeighbor, len_of_neigh, len_of_test, str(sys.argv[6]), LinkDelay,
              NodeDelay, IncomingLimit, NodeHash, hist_score_table, hist_score_length,
                NeighborSets, IncomingNeighbor)
    # subset algorithm
